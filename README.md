...
*Add gene name.* For many applications, it would be desirable to associate the official (annotated) gene name with each transcript. PsiCLASS provides the program "add-genename" for such purpose. "add-genename" takes as input the GTF of gene annotation and a file listing the raw GTF files as input. It will generate a new GTF file for each input raw GTF file by appending the canonical gene names. If a gene is not found in the annotation, "add-genename" will use "novel_INT" to represent its gene name. 
